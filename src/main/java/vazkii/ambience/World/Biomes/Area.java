...
*/
	...
	public static Area getBlockStandingArea(BlockPos pos) {	
		...
		if (Ambience.getWorldData().listAreas != null) {
			...
			for (Area area : Ambience.getWorldData().listAreas) {
					...
					Border border = new Border(area.getPos1(), area.getPos2());
					if (border.p1 != null & border.p2 != null)
						if (border.contains(new Vector3d(pos.getX(),pos.getY(),pos.getZ()))) {								
							Areas.add(area);
						}					
			}
		}
	...
	public CompoundNBT SerializeThis() {
		...
		tagCompound.putInt("RedstoneStrength", getRedstoneStrength()); // RedstoneStrength
	...
	public static Area DeSerialize(CompoundNBT nbt) {
		...
		area.setRedstoneStrength(nbt.getInt("RedstoneStrength"));
		...
		Vector3d pos1 = new Vector3d(tagListPos.getDouble("x1"), tagListPos.getDouble("y1"),tagListPos.getDouble("z1"));		
		Vector3d pos2 = new Vector3d(tagListPos.getDouble("x2"), tagListPos.getDouble("y2"),tagListPos.getDouble("z2"));
	...
	public static List<Area> DeSerializeList(CompoundNBT nbt) {
		...
		for (int i = 0; i < nbt.getInt("lenght"); i++) {
			...
			Vector3d pos1 = new Vector3d(tagListPos.getDouble("x1"), tagListPos.getDouble("y1"),tagListPos.getDouble("z1"));area.setPos1(pos1);
			Vector3d pos2 = new Vector3d(tagListPos.getDouble("x2"), tagListPos.getDouble("y2"),tagListPos.getDouble("z2"));
			area.setPos1(pos1);
			area.setPos2(pos2);
