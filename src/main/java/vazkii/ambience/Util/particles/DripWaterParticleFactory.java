...
public class DripWaterParticleFactory extends DripParticle.DrippingWaterFactory {
			...
			if(!worldIn.isRaining()) 
			{
				...
				if (xSpeed == 0 && ySpeed == 0 && zSpeed == 0) {
					...
					if (!worldIn.getBlockState(new BlockPos(x, y + 2, z)).getBlockState().getFluidState().isEmpty()) {
		            	...
		            	BlockRayTraceResult rayTraceResult = worldIn.rayTraceBlocks(new RayTraceContext(vector, vector.add(new Vector3d(0,1,0).scale(-25)),RayTraceContext.BlockMode.OUTLINE,  RayTraceContext.FluidMode.ANY,worldIn.getClosestPlayer(x, y, z,10,true)));
	...
	public void wrap(DripParticle.DrippingWaterFactory real) {
		...
		IAnimatedSprite spr = ObfuscationReflectionHelper.getPrivateValue(DripParticle.DrippingWaterFactory.class, real,"field_217522_a");
		ObfuscationReflectionHelper.setPrivateValue(DripParticle.DrippingWaterFactory.class, this, spr,"field_217522_a");
